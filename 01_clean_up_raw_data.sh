...
cd ~/Documents/TFM/OC_TFM/data/fastq #Cambia el directorio actual al camino especificado, que es "~/Documents/TFM/OC_TFM/data/fastq". Esto es donde se encontrarán los archivos de datos de entrada.
...
# dentro del bucle, se ejecuta el comando fastp para preprocesar los archivos de datos de cada muestra. 
   ...
   #‘-f’ elimina las 5 bases del frente para la lectura 1 (forward read); ‘-F’ elimina las 25 bases del frente para la lectura 2 (reverse read); ‘-w’ asigna 16 hilos/threads de trabajo para el procesamiento; ‘-5’ inicia una ventana deslizante desde el frente (5') hasta la cola, eliminando las bases dentro de la ventana si su calidad promedio bajo por debajo de 30, y deteniendo el procesamiento en caso contrario; ‘-3’ inicia una ventana deslizante desde la cola (3') hasta el frente, eliminando las bases en la ventana si su calidad promedio baja por debajo de 30, y deteniendo el procesamiento en caso contrario; ‘-l’ descarta las lecturas más cortas de 30 pares de bases de longitud
   -V "$SAMPLE".loginfo -h "$SAMPLE".html #‘-V’ genera información de registro detallada, almacenada en un archivo llamado "$SAMPLE".loginfo; ‘-h’ indica el nombre del archivo de informe en formato HTML como "$SAMPLE".html.
done
