...
cat sample_rg_fields.txt | while read SAMPLE LB PU ID #Este comando lee el contenido del archivo "sample_rg_fields.txt" línea por línea y divide cada línea en cuatro partes: SAMPLE (nombre de la muestra), LB (identificador de la biblioteca), PU (unidad de plataforma), e ID (identificador de grupo de lecturas).
do
  ...
  --RGPU "$PU" \ #RGPU: platform unit. [FLOWCELL].[LANE].[SAMPLE BARCODE] -> Define la unidad de plataforma (platform unit) para el grupo de lecturas. La estructura de PU es "[FLOWCELL].[LANE].[SAMPLE BARCODE]"
